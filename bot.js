...
function getElProfesore(players) {
  ...
  if (!filtered.length) filtered = [players[players.length-1]];
...
function getRandomImage(isWin) {
  ...
  let img;
  do {
    img = valid[Math.floor(Math.random() * valid.length)];
  } while(img === lastImage && valid.length > 1);
...
async function processMatch(nick, forceSend = false, interaction = null) {
  try {
    ...
    const resultText = `🟢 WIN | 🔴 LOSE: ${our}:${enemy}`;
    ...
    const matchDate = new Date(lastMatch.started_at*1000);
    const dateStr = `${matchDate.getDate().toString().padStart(2,'0')}.${(matchDate.getMonth()+1).toString().padStart(2,'0')}.${matchDate.getFullYear()}, ${matchDate.getHours().toString().padStart(2,'0')}:${matchDate.getMinutes().toString().padStart(2,'0')}`;

    const message = `📊 Raport ${mentions}
📅 Data: ${dateStr}
🌍 Mapa: ${lastMatch.map}
🐐 GOAT: ${top.nick} (${top.kills})
🚑 PROFESORE: ${profesore.nick} (${profesore.kills})${eloLines}
📋 OUR:
...
${formatPlayerStats(enemyTeam?.players)}`;
    ...
    if (interaction) {
      await interaction.reply({ content: message, files: image ? [image] : [] });
    } else {
...
client.once('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('zmecz')
      .setDescription('Sprawdza mecz gracza Zweih')
  ];
  ...
  // Po uruchomieniu bot wysyła ostatni mecz
  for (const n of nicknames) {
...
client.on('interactionCreate', async interaction => {
  ...
  } else if (interaction.commandName === 'zmecz') {
    await processMatch('Zweih', true, interaction);
  }
});

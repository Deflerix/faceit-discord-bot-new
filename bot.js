...
async function processMatch(nick, forceSend = false, interaction = null) {
  try {
    ...
    const playersResults = team.players.map(p => {
      ...
      return `${p.nickname} K/D: ${s.Kills || 0}/${s.Deaths || 0} K/Dśr: ${kdRatio} ADR: ${adr} HS%: ${s["Headshots %"] || "-"}`;
    }).join("\n");

    const eventTime = new Date(lastMatch.finished_at || lastMatch.started_at || Date.now())
      .toLocaleString('pl-PL', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });

    const message = `📊 Raport z Faceit ${trackedNicks.map(n => getMention(n)).join(" ")}
